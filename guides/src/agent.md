...
> **The inference boundary and the loop around it.** A `ProviderInterface` is the one pluggable LLM contract — turn a conversation (plus optional tool definitions from [`@orkestrel/tool`](tool.md)) into either an assembled `ProviderResult` (`generate`) or a stream of channel-tagged `ProviderDelta`s that RETURNS the same assembled result (`stream`). This package owns the abstract boundary, conversations, prompt assembly, authority, durable job wiring, and the agent loop that drives context → provider → tools → repeat. Any backend that implements `ProviderInterface` drops in unchanged; a concrete provider is supplied by the host application.
>
> **What it deliberately is NOT.** This is a lean kit of composable primitives, not a heavyweight agent framework. There is no hidden global state, plugin lifecycle, prompt-template DSL, or implicit memory store. Tool execution belongs to `@orkestrel/tool`; files and workspace persistence belong to [`@orkestrel/workspace`](workspace.md). The agent consumes both packages directly. Tools are advertised STRUCTURALLY (`tools.definitions()`) and never serialized into the prompt. A turn is bounded by ONE plain `AbortSignal` (a cancel / [timeout](timeout.md) / [budget](budget.md) folded via `AbortSignal.any`) rather than a bespoke cancellation system.
>
> **The load-bearing guarantees.** A turn is always BOUNDED (`signal` + `timeout` + `budget` via `AbortSignal.any`) and always TERMINATES (tool iteration capped at `limit`). A cancel is not an error: it commits a PARTIAL `AgentResult` that RESOLVES (`partial: true`), so only a genuine provider/tool failure rejects. `generate` and `stream` share ONE private `#run`, so the one-shot result cannot diverge from the live stream. Tool failures arrive as `ToolResult` values narrowed on `success`; a scoped-out tool is neither advertised nor callable. A buggy push observer cannot corrupt the run because the emitter isolates listener throws. Source: [`src/core`](../../src/core), surfaced through the `@src/core` barrel.
>
> **Workspace integration.** `AgentContext` accepts a `WorkspaceManagerInterface` from `@orkestrel/workspace`. Its ACTIVE workspace is the sole document/image context: `build()` renders text files under `## Workspace` with the agent-owned `fencedFile` framing and attaches binary files whose MIME begins with `image/` to the last user turn. Workspace creation, editing, events, snapshots, and stores remain documented and exported by `@orkestrel/workspace`; agent owns only this prompt-carrier policy.

## Surface

This module is the ABSTRACT inference boundary plus the conversation, context, authority, job, and loop machinery around it. Tool and workspace entities come from their originating packages and are consumed directly; they are not re-exported here. A concrete provider implementing `ProviderInterface` is supplied by the host application.
...
Register callable tools through `@orkestrel/tool`. `definitions()` yields the `ToolDefinition[]` to hand the provider; when the model emits a `ToolCall`, `execute` returns a `ToolResult` discriminated by `success`. Handler throws and unknown names become failure results, and batch execution isolates each call:
...
`schema`, like `think`, rides into `provider.stream` as a `ProviderStreamOptions` -- the loop composes both into ONE options object, omitting whichever key is unset (an options object is passed only when at least one of `think` / `schema` is present, preserving the prior think-only behavior when neither is given).
...
Register a conversation in a `ConversationManager` and pass that registry through `AgentContextOptions.conversations` to make it the message source. `context.messages` then is the active conversation's live tail, and `build()` folds its `view()`; the conversation owns message inclusion via compaction, so the scope's `messages` allow-list is not applied. When the registry is omitted, the context creates one with an active default conversation.

Pass that registry through `AgentOptions.conversations` together with an `AgentOptions.window` context [`Budget`](budget.md) to enable automatic compaction. Each turn the loop estimates the current full prompt against the window and, when it reaches the ceiling, compacts the active summarizable conversation before continuing on the rebuilt view. Omit `window` and the loop does not auto-compact. See the [Contract](#contract) (clause 24) for the exact trigger and single-level limitation.
...
| `AgentContext`              | class | The richer turn context — `system` + the instruction manager + the workspace registry (the SOLE document/image context) + `messages` + `tools` + a readonly `scope` changed through `apply`; `build()` folds the scoped managers + the active workspace into one system block then the scoped conversation (tools advertised structurally).                                                                                                                                                                                                             |
...
| `estimateMessages`       | function | Estimate a message batch's footprint — content + `MESSAGE_TOKEN_OVERHEAD` per message + a tool-call JSON estimate + `IMAGE_TOKEN_ESTIMATE` per image; the default `consume` for an agent's context `window` budget. Total — never throws, including on a circular `ToolCall.arguments` (falls back to a fixed contribution instead of the unreachable JSON length). The constants (`MESSAGE_TOKEN_OVERHEAD`, `IMAGE_TOKEN_ESTIMATE`, the `ceil(length / 4)` char heuristic) are deliberate, provider-agnostic APPROXIMATIONS, not an exact tokenizer count — actual window-sizing accuracy depends on the target model's own tokenization, so a caller wanting a sharper count supplies its own `consume` to `createBudget`. |
| `sanitizeToken`          | function | Normalize one reported token count — non-finite / non-positive values become `0`; positive fractional values floor down.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `sanitizeUsage`          | function | Normalize a provider-reported `TokenUsage` — non-finite / negative fields floor to `0`, fractional fields floor down; applied automatically to BOTH a normal turn's `result.usage` and an abort's partial usage before either is charged/folded, so a buggy provider's dirty usage can never poison budget accounting.                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `handleAgentQueueJob`    | function | Rehydrate one queued job with its attempt signal and settle it through the shared partial policy; the named handler composed by `createAgentQueue`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `handleAgentRunnerJob`   | function | Fan out a runner job's declared children without inline-awaiting them, then rehydrate and settle its parent through the shared partial policy; the named handler composed by `createAgentRunner`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `ConversationOptions`          | interface | `{ id?; on?; summarize?; keep?; sections? }` — `createConversation` configuration (the seam + retained-tail size + an optional `>= 1` cap on the compacted `sections` list; absent `summarize` ⇒ `compact()` throws; a sub-1 `sections` throws `ConversationError('SECTIONS')`).                                                                                                                                                                                                                                                                                                  |
| `CompactOptions`               | interface | `{ keep?; sections? }` — per-compaction overrides for `Conversation.compact()` (override the retained-tail size and/or the `sections` cap for ONE fold).                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
Agent-owned readonly data members remain in the Surface tables above; their call-signature methods are documented below. Tool contracts resolve to `/tool`, while workspace contracts resolve to `/workspace`; neither dependency surface is duplicated or re-exported here. `AgentContextInterface.tools` is loop advertising/dispatch machinery and never a prompt-rendering manager.

## Methods

The tables list every public call-signature member of `ProviderInterface`, `ThinkSplitterInterface`, `MessageManagerInterface`, `InstructionManagerInterface`, `ScopeInterface`, `ScopeManagerInterface`, `AgentContextInterface`, `AgentInterface`, `AuthorityInterface`, `AgentRegistryInterface`, `ConversationInterface`, and `ConversationManagerInterface`. Their readonly data members remain Surface rows. `ThinkSplitter`, `InstructionManager`, `Scope`, `ScopeManager`, `AgentContext`, `Agent`, `Authority`, `AgentRegistry`, `Conversation`, and `ConversationManager` implement their interfaces exactly, so the tables also describe those classes' instance methods. `MessageManagerInterface` has no separate concrete class here: the active `Conversation` satisfies it structurally. A host application supplies the concrete `ProviderInterface`. Tool and workspace methods live in their dependency guides. `StreamInterface<T, R>` is the generic live handle (`events`, `result`, and `abort(reason?)`); `AgentStreamInterface` specializes it for `AgentChunk` and `AgentResult`.
...
12. **The `AgentChunk` stream.** `stream().events` yields, per turn, each content delta as `{ type: 'token', content }`, each live reasoning delta as `{ type: 'think', content }`, then optional usage, then one `{ type: 'tool', call, result }` per dispatched call. The call and discriminated result use the contracts imported from `@orkestrel/tool`. `result` resolves the settled `AgentResult`: final or partial content, joined thinking, summed optional usage, and `partial`.
...
### Surfacing a workspace into an agent (read); driving it (edit) via `@orkestrel/tool`
...
The EDIT surface — a MANAGER-DRIVEN, `operation`-keyed `ToolInterface` wired onto `agent.context.tools` over the SAME `context.workspaces` registry — is published by `@orkestrel/tool`; see that package's docs for wiring it in and for its dispatch / error semantics.

### Switching which workspace the model sees (`context.workspaces`)

Only the ACTIVE workspace renders — the other registered workspaces never reach the model. `switch` changes which workspace the model sees between runs:
...
- **Keep workspace ownership at the dependency boundary** — create and edit files through `@orkestrel/workspace`. `AgentContext` consumes its `isText` / `isBinary` guards only to project the active workspace into the prompt; image attachment is the local binary-plus-`image/` MIME policy, not a workspace helper.
- **Surface a workspace into an agent two ways** — `context.workspaces` is the READ surface: its active workspace renders by carrier every turn, active-only and filtered by `scope.files`. The `@orkestrel/tool` workspace-editing integration is the EDIT surface over that same registry. Workspace editing, errors, and persistence remain in [`workspace.md`](workspace.md); tool dispatch semantics remain in [`tool.md`](tool.md).

## Tests

- [`tests/guides/src/parity.test.ts`](../../tests/guides/src/parity.test.ts) — the `## Surface` ↔ `src/core` bijection for agent-owned values and types, plus exhaustive method parity for every agent-owned interface/class pair documented above.
...
- [`tests/src/core/helpers.test.ts`](../../tests/src/core/helpers.test.ts) — agent-owned pure helpers: token/message estimates, allow-list filtering, conversation/message guards, usage sanitation, workspace prompt fencing, and durable-job partial settlement.
- [`tests/src/core/factories.test.ts`](../../tests/src/core/factories.test.ts) — agent-owned factories for conversations, context, the bounded loop, authority, registries, queues, and runners, using real declared tool/workspace dependencies where integration requires them.
...
- [`emitter.md`](emitter.md) — the foundational observable primitive the `Agent` owns as its push `emitter` (the §13 pattern); `AgentEventMap` is its event map, wired via the reserved `on` option.
- [`tool.md`](tool.md) — the originating `@orkestrel/tool` surface consumed for definitions, dispatch, and discriminated results.
- [`workspace.md`](workspace.md) — the originating `@orkestrel/workspace` surface whose active files `AgentContext` projects into provider input.
- [`contract.md`](contract.md) — the shape DSL other tools (e.g. the `@orkestrel/tool` workspace-editing tool) compile against; the shared `describedLiteral` (a discriminant's description-carrier) and `schemaToParameters` (the tool-parameters narrowing) live there.

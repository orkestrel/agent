...
`AgentProvider` is `abstract`, and it is documented through its contract: `AgentProviderInterface` carries the Surface row, and the members a subclass fills are listed under [`## Methods`](#methods).
...
The wire is strictly narrower than the domain on purpose. `providerRequestContract` and `relayFrameContract` are compiled from the shapes in [Shapes and contracts](#shapes-and-contracts), and `RelayProvider.body` refuses a request the JSON wire cannot carry — a non-JSON tool argument, a parameter schema with a synthetic serializer — before it fetches. A `ToolCall.caller` is local context and never crosses the hop.
...
| `DEFAULT_PROVIDER_TIMEOUT`  | const | `number` | Holds the default provider deadline in milliseconds.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `MAX_ERROR_BODY_LENGTH`     | const | `number` | Bounds the decoded error excerpt's input in bytes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `DEFAULT_RELAY_LIMIT`       | const | `number` | Holds the default relay request limit in bytes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `RELAY_CONTENT_TYPE`        | const | `string` | Names the relay's newline-delimited JSON content type.                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `RELAY_PROVIDER_MESSAGE`    | const | `string` | Names the public message for an unexpected upstream relay failure.                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `UNAUTHORIZED_RELAY_STATUS` | const | `number` | Rejects a relay request whose authorization does not succeed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `INVALID_RELAY_STATUS`      | const | `number` | Rejects a relay request whose body fails validation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `OVERSIZED_RELAY_STATUS`    | const | `number` | Rejects a relay request that exceeds its byte budget.                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `UPSTREAM_RELAY_STATUS`     | const | `number` | Identifies a relay provider call that could not be constructed.                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
A contract's `is` narrows an unknown record to its wire shape and its `parse` projects one, so the same declaration guards an inbound body and a frame written back out:
...
if (providerRequestContract.is(body)) {
	upstream.stream(body.messages, signal, body.tools, body.options)
}
...
The framing seam a concrete provider hands the engine, one fresh instance per call, so no call inherits another's half-read record. The engine feeds each decoded chunk to `parse` and clears the parser on every exit.
...
1. **Doc ↔ source bijection.** Every `function` / `class` / `const` / `interface` / `type` row in the `## Surface` tables is a real export of `src/core`, and every export appears as a Surface row — exhaustive, both directions. An `abstract class` sits outside the reflection grammar that gate reads, so `AgentProvider` carries no Surface row of its own: its contract `AgentProviderInterface` carries one and its members are documented under [`## Methods`](#methods).
...
A browser must never hold a model credential. Mount `createRelay` on your own server, where the credential already lives, and give the browser `createRelayProvider` pointed at that route: the browser drives a `ProviderInterface` like any other, and the credential never leaves the server. The handler is a plain `(request: Request) => Promise<Response>`, so any fetch-standard router mounts it — this composition mounts it on an `@orkestrel/router` dispatcher.

`@orkestrel/agent` declares no dependency on a newline-delimited JSON parser, so the browser application supplies one — `createNDJSONParser` from `@orkestrel/ndjson` here, and an equivalent parser from this repository's test infrastructure in the executed transcription of this fence in [`tests/guides.test.ts`](../tests/guides.test.ts).

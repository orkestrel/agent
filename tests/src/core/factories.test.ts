...
// arm-constructors createTextContent / createBinaryContent, createWorkspace (a working in-memory
// WorkspaceInterface), and createWorkspaceTool (the agent-facing tool that EDITS a workspace via
// the operation-discriminated contract) (AGENTS �16 — real data, no mocks).
...
describe('createWorkspaceTool', () => {
	// A workspace MANAGER (the tool is manager-driven, dispatching every edit / read op against
	// `manager.active`) with one active workspace seeded with a couple of text files. Each test
	// drives the SAME tool the way the agent loop does — `tool.execute(operation)` — and asserts the
	// returned value AND the ACTIVE workspace's side-effect. The first `add` auto-activates, so the
	// returned `workspace` IS `manager.active`.
	const wired = (): {
		manager: WorkspaceManagerInterface
		workspace: WorkspaceInterface
		tool: ReturnType<typeof createWorkspaceTool>
	} => {
		const manager = createWorkspaceManager()
		const workspace = manager.add() // auto-activates — this IS manager.active
		workspace.write('a.ts', 'const x = 1')
		workspace.write('b.ts', 'const y = 2')
		return { manager, workspace, tool: createWorkspaceTool(manager) }
	}

	it('advertises the operation-union schema as its parameters (default name)', () => {
		const tool = createWorkspaceTool(createWorkspaceManager())
		expect(tool.name).toBe(WORKSPACE_TOOL_NAME)
		expect(isRecord(tool.parameters)).toBe(true)
		// The advertised schema is the anyOf operation union; its splice arm carries the FOUR FLAT int
		// fields (NOT a nested range) — the small-model surface.
		const anyOf = isRecord(tool.parameters) ? tool.parameters.anyOf : undefined
		...
		const keys = isRecord(spliceProps) ? Object.keys(spliceProps).sort() : []
		expect(keys).toEqual([
			'content',
			'fromColumn',
			'fromLine',
	...
	it('read ? returns the file text', async () => {
		const { tool } = wired()
		expect(await tool.execute({ operation: 'read', path: 'a.ts' })).toBe('const x = 1')
		// An absent path reads undefined (no throw — an optional lookup).
		expect(await tool.execute({ operation: 'read', path: 'missing.ts' })).toBeUndefined()
	})

	it('list ? summarizes every file (path / state / size / lines / kind)', async () => {
		const { tool } = wired()
		const listed = await tool.execute({ operation: 'list' })
		expect(listed).toEqual([
			{ path: 'a.ts', state: 'created', size: 11, lines: 1, kind: 'text' },
			{ path: 'b.ts', state: 'created', size: 11, lines: 1, kind: 'text' },
		])
	})

	it('has ? whether a file exists', async () => {
		const { tool } = wired()
		expect(await tool.execute({ operation: 'has', path: 'a.ts' })).toBe(true)
		expect(await tool.execute({ operation: 'has', path: 'nope.ts' })).toBe(false)
	})

	it('search ? returns the matches', async () => {
		const { tool } = wired()
		const matches = await tool.execute({ operation: 'search', query: 'const' })
		expect(Array.isArray(matches)).toBe(true)
		const hits: readonly ToolResult[] = Array.isArray(matches) ? matches : []
		expect(hits.length).toBe(2)
		expect(isRecord(hits[0]) && hits[0].path).toBe('a.ts')
		expect(isRecord(hits[0]) && hits[0].line).toBe(1)
	})

	it('replace ? applies the replacement and returns the tally', async () => {
		const { workspace, tool } = wired()
		const tally = await tool.execute({ operation: 'replace', query: 'const', replacement: 'let' })
		expect(tally).toEqual({ query: 'const', replaced: 2, files: 2 })
		expect(workspace.read('a.ts')).toBe('let x = 1')
		expect(workspace.read('b.ts')).toBe('let y = 2')
	})

	it('write ? creates / overwrites and read-back reflects it', async () => {
		const { workspace, tool } = wired()
		const result = await tool.execute({ operation: 'write', path: 'notes.txt', content: 'hello' })
		expect(result).toEqual({ path: 'notes.txt', state: 'created' })
		expect(workspace.read('notes.txt')).toBe('hello')
		// Overwriting an existing path transitions to 'modified'.
		const overwrite = await tool.execute({ operation: 'write', path: 'a.ts', content: 'changed' })
		expect(overwrite).toEqual({ path: 'a.ts', state: 'modified' })
		expect(workspace.read('a.ts')).toBe('changed')
	})

	it('splice ? lands the flat range edit (the four ints reassembled)', async () => {
		const { workspace, tool } = wired()
		// Replace the '1' at line 1, column 11 (inclusive) ? column 12 (exclusive) with '0'.
		...
		expect(result).toEqual({ path: 'a.ts', state: 'modified' })
		expect(workspace.read('a.ts')).toBe('const x = 0')
	})

	it('prepend / append ? land at the ends', async () => {
		const { workspace, tool } = wired()
		expect(
			await tool.execute({ operation: 'prepend', path: 'a.ts', content: '// head\n' }),
		).toEqual({
			path: 'a.ts',
			state: 'modified',
		})
		expect(await tool.execute({ operation: 'append', path: 'a.ts', content: '\n// tail' })).toEqual(
			{
				path: 'a.ts',
				state: 'modified',
			},
		)
		expect(workspace.read('a.ts')).toBe('// head\nconst x = 1\n// tail')
	})

	it('move ? re-keys the file and reports moved', async () => {
		const { workspace, tool } = wired()
		const result = await tool.execute({ operation: 'move', from: 'b.ts', to: 'c.ts' })
		expect(result).toEqual({ from: 'b.ts', to: 'c.ts', moved: true })
		expect(workspace.has('b.ts')).toBe(false)
		expect(workspace.read('c.ts')).toBe('const y = 2')
	})

	it('remove ? drops the file and reports removed', async () => {
		const { workspace, tool } = wired()
		const result = await tool.execute({ operation: 'remove', path: 'a.ts' })
		expect(result).toEqual({ path: 'a.ts', removed: true })
		expect(workspace.has('a.ts')).toBe(false)
		expect(await tool.execute({ operation: 'remove', path: 'gone.ts' })).toEqual({
			path: 'gone.ts',
			removed: false,
		})
	})

	// -- Error isolation: through a REAL ToolManager, every failure lands in ToolResult.error
	// (the canonical wrap ? MCP isError) — NOT a throw. The handler RETURNS on success and THROWS a
	// typed WorkspaceError on failure; the ToolManager isolates the throw.
	const through = async (
		tool: ReturnType<typeof createWorkspaceTool>,
		...
		const manager = createToolManager()
		manager.add(tool)
		return manager.execute({ id: 'call-1', name: WORKSPACE_TOOL_NAME, arguments: args })
	}

	it('an invalid regex search ? PATTERN, isolated into ToolResult.error (no throw)', async () => {
		const { tool } = wired()
		const result = await through(tool, { operation: 'search', query: '(', regex: true })
		expect(result.error).toBeDefined()
		expect(result.value).toBeUndefined()
	})

	it('an invalid regex replace ? PATTERN, isolated into ToolResult.error', async () => {
		const { tool } = wired()
		const result = await through(tool, {
			operation: 'replace',
			query: '(',
			replacement: 'x',
			regex: true,
	...
	it('an inverted / sub-1 splice range ? RANGE, isolated into ToolResult.error', async () => {
		const { tool } = wired()
		// Inverted: start (line 2) after end (line 1).
		const result = await through(tool, {
			operation: 'splice',
			path: 'a.ts',
			content: 'x',
	...
	it('prepend / splice on a binary file ? MODALITY, isolated into ToolResult.error', async () => {
		// Seed a binary file via the manager's `add({ seed })` (auto-activates), then drive the tool
		// over the manager so the op targets that active workspace.
		const image = createFile({
			path: 'icon.png',
			content: createBinaryContent('AAAA', 'image/png'),
		})
		const manager = createWorkspaceManager()
		manager.add({ seed: [['icon.png', image]] })
		const tool = createWorkspaceTool(manager)
		const prepend = await through(tool, { operation: 'prepend', path: 'icon.png', content: '//' })
		expect(prepend.error).toBeDefined()
		expect(prepend.value).toBeUndefined()
		const splice = await through(tool, {
			operation: 'splice',
			path: 'icon.png',
			content: 'x',
			fromLine: 1,
		...
		expect(splice.error).toBeDefined()
		expect(splice.value).toBeUndefined()
	})

	it('an unknown / missing operation ? TOOL, isolated into ToolResult.error', async () => {
		const { tool } = wired()
		const unknown = await through(tool, { operation: 'destroy', path: 'a.ts' })
		expect(unknown.error).toBeDefined()
		expect(unknown.value).toBeUndefined()
		const missing = await through(tool, { path: 'a.ts' })
		expect(missing.error).toBeDefined()
		expect(missing.value).toBeUndefined()
	})

	// The handler is SYNCHRONOUS (every workspace op is sync), so a direct `tool.execute(badArgs)`
	// THROWS synchronously (the ToolManager still isolates it — it awaits the result — as the
	// `through` cases above prove). Assert the thrown WorkspaceError's `code` directly.
	const codeOf = (run: () => unknown): string | undefined => {
		try {
			run()
		} catch (error) {
			return isWorkspaceError(error) ? error.code : 'NOT_A_WORKSPACE_ERROR'
		}
		return undefined
	}

	it('tool.execute rejects a malformed operation directly with a TOOL WorkspaceError', () => {
		const { tool } = wired()
		expect(codeOf(() => tool.execute({ operation: 'nope' }))).toBe('TOOL')
	})

	it('tool.execute propagates a RANGE WorkspaceError directly (the ToolManager isolates it for callers)', () => {
		const { tool } = wired()
		expect(
			codeOf(() =>
				tool.execute({
	...
	// -- Manager-driven dispatch (W-g): every edit / read op targets manager.active -----------
	it('an edit op lands on the manager’s ACTIVE workspace (not an inactive one)', async () => {
		const manager = createWorkspaceManager()
		const first = manager.add() // auto-activates
		const second = manager.add() // NOT active (a later add leaves active unchanged)
		const tool = createWorkspaceTool(manager)
		await tool.execute({ operation: 'write', path: 'notes.txt', content: 'hello' })
		// The active (first) workspace reflects the edit; the inactive (second) never sees it.
		expect(first.read('notes.txt')).toBe('hello')
		expect(second.has('notes.txt')).toBe(false)
	})

	// -- No-active rule: a WRITING op auto-creates + activates a default workspace -------------
	it('a write with NO active workspace auto-creates + activates one (then edits land there)', async () => {
		const manager = createWorkspaceManager()
		expect(manager.active).toBeUndefined()
		const tool = createWorkspaceTool(manager)
		const result = await tool.execute({ operation: 'write', path: 'first.txt', content: 'seeded' })
		expect(result).toEqual({ path: 'first.txt', state: 'created' })
		// A default workspace now exists AND is active, carrying the edit.
		expect(manager.count).toBe(1)
		expect(manager.active).toBeDefined()
		expect(manager.active?.read('first.txt')).toBe('seeded')
	})

	// -- No-active rule: a pure-READ op returns the empty result without creating one ----------
	it('a read / list / has / search with NO active workspace returns empty (no workspace created)', async () => {
		const manager = createWorkspaceManager()
		const tool = createWorkspaceTool(manager)
		expect(await tool.execute({ operation: 'read', path: 'x.txt' })).toBeUndefined()
		expect(await tool.execute({ operation: 'has', path: 'x.txt' })).toBe(false)
		expect(await tool.execute({ operation: 'list' })).toEqual([])
		expect(await tool.execute({ operation: 'search', query: 'anything' })).toEqual([])
		// None of the read ops created a workspace — the registry is still empty + inactive.
		expect(manager.count).toBe(0)
		expect(manager.active).toBeUndefined()
	})

	// -- switch: re-points active so a SUBSEQUENT edit hits the new active workspace -----------
	it('switch re-points active and reports the now-active workspace (id + files)', async () => {
		const manager = createWorkspaceManager()
		const a = manager.add() // auto-activates
		a.write('a.ts', 'const x = 1')
		const b = manager.add()
		b.write('b1.ts', 'const y = 2')
		b.write('b2.ts', 'const z = 3')
		const tool = createWorkspaceTool(manager)
		// Switch active ? b; the result reports b's id + its file count.
		const switched = await tool.execute({ operation: 'switch', id: b.id })
		expect(switched).toEqual({ id: b.id, switched: true, files: 2 })
		expect(manager.active?.id).toBe(b.id)
		// A subsequent edit now lands on b (the new active), not a.
		await tool.execute({ operation: 'write', path: 'fresh.txt', content: 'on b' })
		expect(b.read('fresh.txt')).toBe('on b')
		expect(a.has('fresh.txt')).toBe(false)
	})

	it('switch to an unknown id is lenient — reports switched:false and leaves active unchanged', async () => {
		const { manager, workspace, tool } = wired()
		const before = manager.active?.id
		const result = await tool.execute({ operation: 'switch', id: 'no-such-id' })
		expect(result).toEqual({ id: 'no-such-id', switched: false })
		// Active is unchanged (still the originally-active workspace).
		expect(manager.active?.id).toBe(before)
		expect(manager.active?.id).toBe(workspace.id)
	})

	// -- Discovery: the `workspaces` op lists each registered workspace for the model to switch to -
	it('workspaces lists every registered workspace with { id, files, active }', async () => {
		const manager = createWorkspaceManager()
		const a = manager.add() // auto-activates
		a.write('a.ts', 'const x = 1')
		const b = manager.add()
		...
		b.write('b2.ts', 'const z = 3')
		const tool = createWorkspaceTool(manager)
		const listed = await tool.execute({ operation: 'workspaces' })
		expect(listed).toEqual([
			{ id: a.id, files: 1, active: true },
			{ id: b.id, files: 2, active: false },
		])
	})

	it('workspaces is [] when the registry is empty', async () => {
		const manager = createWorkspaceManager()
		const tool = createWorkspaceTool(manager)
		expect(await tool.execute({ operation: 'workspaces' })).toEqual([])
	})
})
